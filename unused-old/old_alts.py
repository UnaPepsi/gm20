...
def get_acc(endpoint: str) -> str:
	if endpoint == "minecraft":
		...
		url = f'{endpoint}/'
	response = requests.post(f'https://altsforyou.org/{url}', headers=headers, data=data)
	link = ""
	for i in response.text[response.text.find("http://test.shrinkurl.org/"):]:
		if i == '"':
			break
		link += i
	return link